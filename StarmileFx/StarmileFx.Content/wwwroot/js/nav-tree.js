...
//		window.parent.window.location = window.parent.window.location.href;//整个页面重载
	...
	if(trreInfo){
        ...
        this.hostName = '';//(this.tool.isContain({arr:['192.168.1.102','127.0.0.1','localhost','192.168.2.168','test.banggood.cn'],str:location.hostname})?trreInfo.hostName:''); 
		//滚动条
...
DropDownMenuTree.prototype.Initialization=function() {//初始化
	...
	if(typeof self.data=="object"?self.data.important:false){
		...
		$.ajax({
		    url:self.hostName+'/Home/GetMenuJson',
		    dataType:'jsonp',
		    jsonp:'callback', 
		...
		Rec1:function(json1,json2){
			...
			$.map(json1, function(a1,i) {
				$.map(json2, function(a2,j) {
					if(a2.text &&(a1.text.toLocaleLowerCase().replace(/[\ ]/g, "") == (a2.text.indexOf('(OA)')>=0?a2.text.split('(OA)')[1]:a2.text).toLocaleLowerCase().replace(/[\ ]/g, "")) ){
//						console.log('站点容器：'+a1.text+'  链接: '+a1.href);
//						console.log('OA系统：'+a2.text+'  链接:'+a2.href);
						if(!a1.href&&!a2.href&&a2.children&&a2.children.length>0){
							json1[i].children=f.Rec1(a1.children,a2.children).str1;
						}else if(json2[i]&&json2[i].text){
							if(json2[i].text.substr(0,4)!="(OA)"){json2[i].text = '(OA)'+json2[i].text}
						}
						if(a2.href&&a2.children&&a2.children.length<=0){
							if(a1.href!=a2.href){
								hterf1 = a1.href;
								hterf2 = a2.href;
								if(hterf2.indexOf('/sso/home/RedirectLoginBG?RedirectURL=')>=0){
									hterf2 = hterf2.split('/sso/home/RedirectLoginBG?RedirectURL=')[1].replace(/[\//]/g, "/").replace(/[\#]/g, "");
								}
								if(hterf2.indexOf('?menuKey=')>=0){
									hterf2 = hterf2.split('?menuKey=')[0].replace(/[\//]/g, "/").replace(/[\#]/g, "");
									hterf2 = (hterf2.substr(0,1)=='/'?hterf2.substr(1,hterf2.length):hterf2).replace(/[\//]/g, "/").replace(/[\#]/g, "");
								}
								if(hterf1.indexOf('?menuKey=')>=0){
									hterf1 = hterf1.split('?menuKey=')[0].replace(/[\//]/g, "/").replace(/[\#]/g, "");
									hterf1 = (hterf1.substr(0,1)=='/'?hterf1.substr(1,hterf1.length):hterf1).replace(/[\//]/g, "/").replace(/[\#]/g, "");
								}
								if(hterf2.indexOf('&menuKey=')>=0){
									hterf2 = hterf2.split('&menuKey=')[0].replace(/[\//]/g, "/").replace(/[\#]/g, "");
									hterf2 = (hterf2.substr(0,1)=='/'?hterf2.substr(1,hterf2.length):hterf2).replace(/[\//]/g, "/").replace(/[\#]/g, "");
								...
								if(hterf2.indexOf(hterf1)<0){
									if(hterf1.indexOf(hostDomain+'/')>=0){
										hterf1=hterf1.split(hostDomain+'/')[1].replace(/[\//]/g, "/").replace(/[\#]/g, "");
										hterf1 = (hterf1.substr(0,1)=='/'?hterf1.substr(1,hterf1.length):hterf1).replace(/[\//]/g, "/").replace(/[\#]/g, "");
									}
									if(hterf2.indexOf(hostDomain+'/')>=0){
										hterf2=hterf2.split(hostDomain+'/')[1].replace(/[\//]/g, "/").replace(/[\#]/g, "");
										hterf2 = (hterf2.substr(0,1)=='/'?hterf2.substr(1,hterf2.length):hterf2).replace(/[\//]/g, "/").replace(/[\#]/g, "");
									}
									if(hterf2.indexOf(hterf1)<0&&hterf2.length>0){json1.push(a2)}
								}
							}
						}
						json2[j]='';
			...
			$.map(json2, function(item,index) {
				if(item.text&&item.text.substr(0,4)!="(OA)"){item.text='(OA)'+item.text}
	...
	var myFun = {
		...
		system:function(){
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                thisText = self.getThisText({ val: searVal, texts: (data[i].text ? data[i].Name : ""), level: "sys", index: [i] });
                ...
                navHtml += '<span class="system-nav-text" title="' + (data[i].text ? data[i].Name : "") + '">' + thisText + '</span>';
		...
		swithBack:function(back) {//皮肤切换
			$(window.document).on('click','.switch-color',function(){
				...
				var links = document.createElement('link');
					...
					links.href = (self.tool.isContain({arr:['127.0.0.1','localhost','192.168.2.85'],str:location.hostname})?"./css/":"https://content.banggood.cn/Content/lib/StandingCont_1_0/") + this_.attr('signback')+".css";
