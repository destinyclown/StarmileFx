...
namespace StarmileFx.Web.Server.Service
{
    public class YoungoManager: IYoungoServer
    {
        ...
        public Task<ResponseResult<List<Product>>> GetProductList(ProductSearch search)
        {
            return Task.Run(() =>
            {
        ...
        public async Task<ResponseResult<List<Product>>> GetProductListAsync(ProductSearch search)
        {
            ...
            {
                return JsonConvert.DeserializeObject<ResponseResult<List<Product>>>(result);
            });
        }
    }
}
