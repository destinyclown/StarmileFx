...
IF NOT EXISTS `OffLineOrderParent` (
	...
	`UpdateTime` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
	`PayTime` TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '付款时间',
	`CreatTime` TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间'
);
...
IF NOT EXISTS `CustomerSign` (
	...
	`CreatTime` TIMESTAMP DEFAULT CURRENT_TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间'
);
...
IF NOT EXISTS `Resources` (
	`ID` INT (11) NOT NULL auto_increment PRIMARY KEY,
	`ProductID` varchar(50) NULL COMMENT '商品ID',
	`ResourcesCode` varchar(20) NULL COMMENT '资源标识',
	`Address` varchar(200) NULL COMMENT '关键字',
	`Type` INT NULL COMMENT '类型',
